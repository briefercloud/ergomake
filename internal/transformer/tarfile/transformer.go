...
type tarfileTransformer struct {
...
func (c *tarfileTransformer) Prepare(ctx context.Context, id uuid.UUID) (*transformer.PrepareResult, error) {
	...
	loadErgopackResult, err := c.loadErgopack(ctx, id)
	if err != nil {
...
func (c *tarfileTransformer) Cleanup() {
	if c.cleanup != nil {
		c.cleanup()
	}
}

func (c *tarfileTransformer) fail(origErr error) error {
...
func (c *tarfileTransformer) Transform(ctx context.Context, id uuid.UUID) (*transformer.TransformResult, error) {
	if !c.prepared {
	...
	buildImagesRes, err := builder.BuildImagesFromTar(ctx, namespace, c.tarpath)
	if err != nil {
...
func (c *tarfileTransformer) makeClusterObjects(ctx context.Context, namespace string) ([]runtime.Object, error) {
...
func (c *tarfileTransformer) saveServices(ctx context.Context, envID uuid.UUID, compose *transformer.Environment) error {
...
func (c *tarfileTransformer) getUrl(service kobject.ServiceConfig) string {
	for _, port := range service.Port {
		if port.HostPort > 0 {
			return strings.ToLower(fmt.Sprintf(
				"%s-%s.%s",
				service.Name,
				c.dbEnvironment.ID.String(),
				clusterDomain,
			))
		}
	}
...
func (c *tarfileTransformer) fixComposeObject(projectPath, namespace string) error {
...
func (c *tarfileTransformer) removeUnsupportedVolumes(service *kobject.ServiceConfig) {
...
func (c *tarfileTransformer) loadErgopack(ctx context.Context, id uuid.UUID) (*LoadErgopackResult, error) {
	projectPath, err := c.extract(ctx, id)
	if err != nil {
		return nil, errors.Wrap(err, "fail to extract tar archive")
	}
	...
	c.cleanup = func() {
		...
		if err != nil {
			logger.Ctx(ctx).Err(err).Str("projectPath", projectPath).Str("id", id.String()).
	...
	validationRes, err := transformer.Validate(c.projectPath)
	if err != nil {
	...
	if c.isCompose {
		...
		err = c.fixComposeObject(projectPath, id.String())
		if err != nil {
...
func (c *tarfileTransformer) transformCompose(ctx context.Context, namespace string) ([]runtime.Object, error) {
...
func (tt *tarfileTransformer) extract(ctx context.Context, id uuid.UUID) (string, error) {
	tmpDir, err := os.MkdirTemp("", fmt.Sprintf("ergomake-%s", id.String()))
	if err != nil {
		return "", errors.Wrap(err, "failed to create temporary directory")
	}

	tarFile, err := os.Open(tt.tarpath)
	if err != nil {
	...
	for {
		...
		case tar.TypeReg:
			...
			if _, err := io.Copy(file, tarReader); err != nil {
				return "", errors.Wrapf(err, "failed to extract file: %s", filePath)
			}
		}
	}
...
func (c *tarfileTransformer) makeEnvironmentFromKObjectServices(komposeServices map[string]kobject.ServiceConfig, rawCompose string) *transformer.Environment {
...
func (c *tarfileTransformer) makeEnvironmentFromErgopack(ctx context.Context, pack *ergopack.Ergopack, rawFile string) *transformer.Environment {
	services := map[string]transformer.EnvironmentService{}
	i := 0
...
func (c *tarfileTransformer) fixOutput(ctx context.Context, objs *[]runtime.Object, namespace string) ([]runtime.Object, error) {
...
func (c *tarfileTransformer) fixDeployment(ctx context.Context, obj runtime.Object) ([]runtime.Object, error) {
...
func (c *tarfileTransformer) fixNamespace(obj runtime.Object, namespace string) {
	objMeta := obj.(metav1.Object)
	objMeta.SetNamespace(namespace)
}

func (c *tarfileTransformer) addSecurityRestrictions(deployment *appsv1.Deployment) {
...
func (c *tarfileTransformer) addNodeContraints(deployment *appsv1.Deployment) {
	podSpec := &deployment.Spec.Template.Spec
...
func (c *tarfileTransformer) fixRestartPolicy(deployment *appsv1.Deployment) {
	podSpec := &deployment.Spec.Template.Spec
...
func (c *tarfileTransformer) fixPullPolicy(deployment *appsv1.Deployment) {
	podSpec := &deployment.Spec.Template.Spec
...
func (c *tarfileTransformer) addResourceLimits(deployment *appsv1.Deployment) {
	podSpec := &deployment.Spec.Template.Spec
	for i := range podSpec.Containers {
		...
		if strings.ToLower(c.gitOptions.Owner) == "writesonic" {
...
func (c *tarfileTransformer) removeHostPort(deployment *appsv1.Deployment) {
	podSpec := &deployment.Spec.Template.Spec
	for i := range podSpec.Containers {
...
func (c *tarfileTransformer) addLabels(deployment *appsv1.Deployment) {
...
func (c *tarfileTransformer) addEnvVars(ctx context.Context, deployment *appsv1.Deployment) (*corev1.Secret, error) {
...
func (c *tarfileTransformer) getSecretForImage(
	ctx context.Context,
	obj runtime.Object,
	namespace string,
...
func (c *tarfileTransformer) getLabels(serviceID, serviceName string) map[string]string {
